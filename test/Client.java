...
public class Client extends javax.swing.JFrame {
	...
	private void jButton1ActionPerformed(java.awt.event.ActionEvent evt)
		...
		fout = new PrintWriter(new BufferedWriter(new OutputStreamWriter(
                ...
                h =new Socket(ip, 9003);
                he=new BufferedReader(new InputStreamReader(fs.getInputStream()));
                hyh=new PrintWriter(new BufferedWriter(new OutputStreamWriter(
				fs.getOutputStream())), true);
	...
	public static void main(String args[]) {
        ...
        private FileManage ht=new FileManage(new File(""));
        private grapthread gt;
        private Socket h;
        private BufferedReader he;
        private PrintWriter hyh;
...
class grapthread extends Thread {
        private Socket hh;
	private BufferedReader hhh;
        ...
        private FileManage ht;

        grapthread(Socket hh,BufferedReader hhh,PrintWriter hhhh,FileManage ht) throws IOException{
            this.hh=hh;
            this.hhh=hhh;
            this.hhhh=hhhh;
            this.ht=ht;
            buffered=new BufferedInputStream(hh.getInputStream());//创建一个缓冲区数组,保存套接字s输入流，以便使用
            din = new DataInputStream(buffered);//数据输入流,用来读取
            dout = new DataOutputStream(hh.getOutputStream());
        }
        ...
        public void run(){
            while (true){
                String istream = null;
            try {
                istream = hhh.readLine();
            } catch (IOException ex) {
                Logger.getLogger(grapthread.class.getName()).log(Level.SEVERE, null, ex);
            }
                if(istream.equalsIgnoreCase("image")){
                    System.out.println("hh");
                snatchScreen ( "c://screen.jpg" );
                
                 ht.openFile( "c://screen.jpg");
                };
                
               try {
                    BufferedInputStream bin = new BufferedInputStream(new FileInputStream("c://screen.jpg"));
                    int l;
